...
impl Pl011Uart {
    ...
    pub fn init(&self) {
        // Disable UART0
        write_reg(self.pl011_virtrange.clone(), UART0_CR, 0);
        ...
        // Clear interrupts
        write_reg(self.pl011_virtrange.clone(), UART0_ICR, 0x7ff);
        ...
        write_reg(self.pl011_virtrange.clone(), UART0_IBRD, int_brd);
        write_reg(self.pl011_virtrange.clone(), UART0_FBRD, frac_brd);

        // Enable FIFOs (tx and rx), 8 bit
        write_reg(self.pl011_virtrange.clone(), UART0_LCRH, 0x70);

        // Mask all interrupts
        write_reg(self.pl011_virtrange.clone(), UART0_IMSC, 0x7f2);

        // Enable UART0, receive only
        write_reg(self.pl011_virtrange.clone(), UART0_CR, 0x81);
    }
    ...
    fn gpiosetpull(&self, pin: u32, pull: GpioPull) {
        ...
        write_reg(self.pl011_virtrange.clone(), GPPUD, pull as u32);
        // ...wait 150 cycles for it to set
        delay(150);
        // ...set the appropriate PUD bit
        write_reg(self.pl011_virtrange.clone(), gppudclk_reg, pud_bit);
        // ...wait 150 cycles for it to set
        delay(150);
        // ...clear up
        write_reg(self.pl011_virtrange.clone(), GPPUD, 0);
        write_reg(self.pl011_virtrange.clone(), gppudclk_reg, 0);
    }
}
...
impl Uart for Pl011Uart {
    fn putb(&self, b: u8) {
        ...
        while read_reg(self.pl011_virtrange.clone(), UART0_FR) & (1 << 5) != 0 {}
        write_reg(self.pl011_virtrange.clone(), UART0_DR, b as u32);
    }
}
