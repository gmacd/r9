...
impl MiniUart {
    ...
    pub fn init(&self) {
        ...
        let mut gpfsel1 = read_reg(self.gpio_virtrange.clone(), GPFSEL1);
        gpfsel1 &= !((7 << 12) | (7 << 15));
        gpfsel1 |= (2 << 12) | (2 << 15);
        write_reg(self.gpio_virtrange.clone(), GPFSEL1, gpfsel1);

        write_reg(self.gpio_virtrange.clone(), GPPUD, 0);
        delay(150);
        write_reg(self.gpio_virtrange.clone(), GPPUDCLK0, (1 << 14) | (1 << 15));
        delay(150);
        write_reg(self.gpio_virtrange.clone(), GPPUDCLK0, 0);

        // Enable mini uart - required to write to its registers
        write_or_reg(self.aux_virtrange.clone(), AUX_ENABLE, 1);
        write_reg(self.miniuart_virtrange.clone(), AUX_MU_CNTL, 0);
        // 8-bit
        write_reg(self.miniuart_virtrange.clone(), AUX_MU_LCR, 3);
        write_reg(self.miniuart_virtrange.clone(), AUX_MU_MCR, 0);
        // Disable interrupts
        write_reg(self.miniuart_virtrange.clone(), AUX_MU_IER, 0);
        // Clear receive/transmit FIFOs
        write_reg(self.miniuart_virtrange.clone(), AUX_MU_IIR, 0xc6);
        ...
        write_reg(self.miniuart_virtrange.clone(), AUX_MU_BAUD, 545);

        // Finally enable transmit
        write_reg(self.miniuart_virtrange.clone(), AUX_MU_CNTL, 3);
    }
}
...
impl Uart for MiniUart {
    fn putb(&self, b: u8) {
        ...
        while read_reg(self.miniuart_virtrange.clone(), AUX_MU_LSR) & (1 << 5) == 0 {
            core::hint::spin_loop();
        }
        write_reg(self.miniuart_virtrange.clone(), AUX_MU_IO, b as u32);
    }
}
