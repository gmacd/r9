...
impl Table {
    ...
    fn next_mut<A: VmTrait>(
        ...
        if !entry.valid() {
            ...
            let page_pa = vm_trait.alloc_physpage();
...
impl RootPageTable {
    ...
    fn map_to<A: VmTrait>(
        ...
        let dest_entry = match page_size {
            PageSize::Page4K => self
                .next_mut(vm_trait, pgtype, Level::Level0, va)
                .and_then(|t1| t1.next_mut(vm_trait, pgtype, Level::Level1, va))
                .and_then(|t2| t2.next_mut(vm_trait, pgtype, Level::Level2, va))
    ...
    pub fn map_phys_range<A: VmTrait>(
        &mut self,
        vm_trait: &mut A,
        debug_name: &str,
        range: &PhysRange,
        ...
        for pa in range.step_by_rounded(page_size.size()) {
            ...
            self.map_to(vm_trait, entry.with_phys_addr(pa), currva, page_size, pgtype)?;
        }
...
impl VmTrait for PhysPageAllocator {
    fn alloc_physpage(&mut self) -> Result<PhysAddr, PageAllocError> {
    ...
    impl TestPageAllocator {
        fn new() -> Self {
            Self {
                ...
                root_page_table: RootPageTable::empty(),
                next_page_idx: 0,
            }
        }
    }

    impl VmTrait for TestPageAllocator {
        fn alloc_physpage(&mut self) -> Result<PhysAddr, PageAllocError> {
